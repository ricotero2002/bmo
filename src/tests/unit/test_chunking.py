...
class DummyAgent:
    ...
    def invoke(self, inputs):
        if self.response_format:
            ...
            if name == "Sentences":
                # Mock returning a list of sentences
                return {
                    "structured_response": DummyAgentResponse(
                ...
                return {
                    "structured_response": DummyAgentResponse(chunk_id=None)
                }
        
        # Default mock 
        return {
            "messages": [
                type("Msg", (object,), {"content": "Resumen falso"}),
                type("Msg", (object,), {"content": "Título falso"})
            ]
        }
...
def test_chunking_router_routes_to_markdown_for_structured_txt(chunking_router, agent_factory):
    ...
    chunks = chunking_router.route_and_split(agent_factory, text, filename="sample_structured.txt")
...
def test_chunking_router_routes_to_agentic_for_short_unstructured_txt(chunking_router, agent_factory):
    ...
    chunks = chunking_router.route_and_split(agent_factory, text, filename="sample_short.txt")
